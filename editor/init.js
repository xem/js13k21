...
editor = () => {
  ...
  resizegrid = e => {
    ...
    for(i = 0; i < size; i++){
      for(j = 0; j < size; j++){
        whtml += `<div class=cell id=w${cellprefix}-${i}-${j} style='width:${cellsize}%;padding-top:${cellsize}%' onmousedown='paint(${i},${j},this,0,0,event);return false;' onmousemove='paint(${i},${j},this,0,1,event)' oncontextmenu='return false;'></div>`;
        ghtml += `<div class=cell id=g${cellprefix}-${i}-${j} style='width:${cellsize}%;padding-top:${cellsize}%' onmousedown='paint(${i},${j},this,1,0,event);return false;' onmousemove='paint(${i},${j},this,1,1,event)' oncontextmenu='return false;'></div>`;
      }
    }
  ...
  paint = (i,j,t,g,f,e) => {
    ...
    // Toggle
      ...
      if(e.button == 0) d[i][j] ^= 1;
      if(e.button == 1) {
        if(portal1placed == 0){
          portal11 = g ? [i,j,0] : [i,0,size-j]; 
          //(g?down:back).insertAdjacentHTML("beforeEnd","toto");
        }
        else if(portal1placed == 1){
          portal12 = g ? [i,j,0] : [i,0,size-j];
          //(g?down:back).insertAdjacentHTML("beforeEnd","toto");
      ...
      if(e.button == 2) {
        if(portal2placed == 0){
          portal21 = g ? [i,j,0] : [i,0,size-j]; 
          (g?down:back).insertAdjacentHTML("beforeEnd","toto");
        }
        else if(portal1placed == 1){
          portal22 = g ? [i,j,0] : [i,0,size-j];
          (g?down:back).insertAdjacentHTML("beforeEnd","toto");          
        }
        portal2placed++;
      }
